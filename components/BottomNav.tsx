...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto grid w-full max-w-md grid-cols-4 px-2 py-1">
        {navItems.map((item) => {
          const isActive =
            item.href === "/"
              ? pathname === "/"
          ...
          return (
            <Link
              ...
              href={item.href}
              className={[
                "flex flex-col items-center justify-center rounded-2xl py-2 text-xs font-semibold transition",
                isActive
