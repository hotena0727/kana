...
export default function KatakanaPage() {
  const [selectedId, setSelectedId] = useState<string>(katakanaList[0]?.id ?? "");
  const [isAutoPlaying, setIsAutoPlaying] = useState(false);
  const [autoPlayMs, setAutoPlayMs] = useState(2000);
  const detailRef = useRef<HTMLDivElement | null>(null);

  const selectedIndex = useMemo(() => {
    const foundIndex = katakanaList.findIndex((item) => item.id === selectedId);
    return foundIndex >= 0 ? foundIndex : 0;
  }, [selectedId]);

  const selectedItem = useMemo(() => {
    return katakanaList[selectedIndex] ?? katakanaList[0];
  }, [selectedIndex]);
  ...
  const handlePrev = () => {
    const prevIndex =
      selectedIndex === 0 ? katakanaList.length - 1 : selectedIndex - 1;
    setSelectedId(katakanaList[prevIndex].id);
    scrollToDetail();
  };

  const handleNext = () => {
    const nextIndex =
      selectedIndex === katakanaList.length - 1 ? 0 : selectedIndex + 1;
    setSelectedId(katakanaList[nextIndex].id);
    scrollToDetail();
  };

  const handleRandom = () => {
    if (katakanaList.length <= 1) return;

    let randomIndex = selectedIndex;
    while (randomIndex === selectedIndex) {
      randomIndex = Math.floor(Math.random() * katakanaList.length);
    }
    setSelectedId(katakanaList[randomIndex].id);
    scrollToDetail();
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setSelectedId((prevId) => {
        const currentIndex = katakanaList.findIndex((item) => item.id === prevId);
        const safeIndex = currentIndex >= 0 ? currentIndex : 0;
        const nextIndex =
          safeIndex === katakanaList.length - 1 ? 0 : safeIndex + 1;
        return katakanaList[nextIndex].id;
      });
    }, autoPlayMs);
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f7fbff_0%,#eef6ff_45%,#f9fcff_100%)] pb-24 text-slate-900">
      <section className="mx-auto w-full max-w-md px-5 py-7">
        <div className="mb-6 rounded-[32px] bg-white/85 p-6 shadow-[0_18px_45px_rgba(15,23,42,0.08)] ring-1 ring-sky-100 backdrop-blur">
          ...
          <h1 className="mt-4 text-[30px] font-extrabold tracking-tight">가타카나</h1>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-slate-500">
                {selectedIndex + 1} / {katakanaList.length}
              </div>
            </div>
