...
import { createKanaQuiz, type QuizMode, type QuizQuestion } from "@/lib/quiz";
...
export default function QuizPage() {
  ...
  useEffect(() => {
    if (!mode) return;
    setQuestions(createKanaQuiz(mode, 10));
  }, [mode]);
  ...
  const startQuiz = (nextMode: QuizMode) => {
    const newQuestions = createKanaQuiz(nextMode, 10);
    setMode(nextMode);
  ...
  const handleRestart = () => {
    ...
    const newQuestions = createKanaQuiz(mode, 10);
    setQuestions(newQuestions);
    setCurrentIndex(0);
    setSelectedChoice(null);
    setIsChecked(false);
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f7fbff_0%,#eef6ff_45%,#f9fcff_100%)] pb-24 text-slate-900">
      <section className="mx-auto w-full max-w-md px-5 py-7">
        ...
        <div className="mb-4 rounded-[28px] bg-white p-4 shadow-[0_14px_36px_rgba(15,23,42,0.07)] ring-1 ring-slate-100">
          <div className="mb-3 inline-flex rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700 ring-1 ring-sky-100">
            {modeLabels[mode]}
          </div>
